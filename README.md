...
```
### Let’s set the variations

Note: Please do not forget that there are places to write names.
```
...
mantrachaind init "node name" --chain-id mantrachain-testnet-1
```
```
...
### wallet
```
mantrachaind keys add wallet-name
```

### Validator Setting
Not: wallet and moniker name 
```
